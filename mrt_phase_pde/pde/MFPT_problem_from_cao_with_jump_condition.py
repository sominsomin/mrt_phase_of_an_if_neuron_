...
jump = np.where((x_all == x[-1]))
right_jump = np.where((x_all == 0.0))
...
for i in range(0, len(x_all)):
    ...
    if i == 0:
        ...
        # idx_left_jump = np.where((x_all == 0.0) & (y_all == y_))
        # row[idx_left_jump] -= 1
        # #
        # # b[i] = 0
        #
...
plt.figure()
plt.contourf(__x, __y, T_matrix.transpose(), levels=100) #, v_min=100, v_max=200)
plt.colorbar()
