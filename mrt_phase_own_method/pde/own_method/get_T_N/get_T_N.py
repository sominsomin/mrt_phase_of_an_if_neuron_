...
from mrt_phase_own_method.pde.own_method.src.exit_point_distribution.exit_point_distribution import ExitPointDistribution
from mrt_phase_own_method.pde.own_method.src.T_0.T_0 import T_0 as T_0_class
from mrt_phase_own_method.pde.own_method.src.T_0.T_N import T_N as T_N_class
from mrt_phase_own_method.pde.own_method.config_T_N import v_min, v_max, a_min, a_max
...
def get_T_N(l_max=None, D=D):
    ...
    T_1 = T_0_class.load(f'..\\data\\T_1_D_{D}_sim_n_thr_1.pickle')
    ...
    if l_max == 1:
        t_n = T_N_class(T_1.v, T_1.a, T_1, N)
        t_n.save(f'..\\result\\T_N_{N}_D_{D}.pickle')
        T_N = T_1.T
        v = T_1.v
        a_new = T_1.a
        ...
        # T N minus 1
        T_N_1 = copy.copy(T_1[np.where(T_1.v == 0)][0, :])

        T_N_all = [T_N_1]

        a_max_temp = np.max(T_1.a)
        a_new = copy.copy(a)
        ...
        for l in range(l_max-2):
            ...
            idx_a = np.where((T_1.a <= Delta_a + a_max_temp) & (T_1.a >= Delta_a))
            ...
            for j, _a in enumerate(a_new):
                ...
                i_0 = np.where(T_1.v == 0)[0][0]
                j_0 = np.where(T_1.a == _a)[0][0]

                T_N_0[j] = T_1[i_0, j_0] + np.sum(p * T_N_1[idx_a]) * np.diff(a_new)[0]
        ...
        for i, _v in enumerate(v):
            for j, _a in enumerate(a_new):
                ...
                i_0 = np.where(_v == T_1.v)[0][0]
                j_0 = np.where(_a == T_1.a)[0][0]

                T_add = np.sum(p * T_N_1[idx_a]) * np.diff(a_new)[0]
                T_N[i, j] = T_1[i_0, j_0] + T_add
    ...
    # det_file_paths = '../../../../mrt_phase_algorithmic/data/results/isochrones/from_timeseries_grid/deterministic/D_0.0'
    # stochastic = f'..\\..\\..\\mrt_phase_algorithmic\\data\\results\\isochrones\\from_timeseries\\stochastic\\D_{D}'
